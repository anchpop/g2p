...
use g2p::{Error, HindiLabels, phonemize, phonemize_lang, phonemize_lang_with, phonemize_language};

#[test]
fn spanish_voice_override_preserves_full_output_and_seseo() {
    let text = "zapato";
    let overridden = phonemize_language("spa", Some("es-419"), text, HindiLabels::Current).unwrap();
    ...
    let default = phonemize_language("spa", None, text, HindiLabels::Current).unwrap();
...
fn assert_backend_identity(lang: &str, text: &str) {
    let result = phonemize_language(lang, None, text, HindiLabels::Current).unwrap();
    assert!(!result.phonemes.is_empty());
    assert_eq!(result, phonemize_lang(lang, text).unwrap());
    assert_eq!(
        result,
        phonemize_lang_with(lang, text, HindiLabels::Current).unwrap()
    );
}
...
fn japanese_without_feature_stays_unsupported() {
    assert!(matches!(
        phonemize_language("jpn", None, "こんにちは", HindiLabels::Current),
...
fn every_backend_rejects_voice_overrides_before_running() {
    for lang in ["hin", "zho-hans", "jpn", "tha", "kor"] {
        assert!(matches!(
            phonemize_language(lang, Some("es-419"), "", HindiLabels::Current),
